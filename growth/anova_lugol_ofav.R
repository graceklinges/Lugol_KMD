...
#for ofav, use delta, for apal use delta (apal study was t0-t3)
...
write.table(omnibus_test, file = "omnibus_tukey_ofav_genotrt_lugol.txt", sep = "\t")
...
write.table(tukey_stats, file = "ofav_SA_tukey_lugol.txt", sep = "\t")
...
kw <- pairwise.wilcox.test(sa$delta, sa$geno_trt,
                           p.adjust.method = "fdr")
write.table(kw$p.value, file = "ofav_SA_kruskal_lugol.txt", sep = "\t")
