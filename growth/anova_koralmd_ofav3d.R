...
write.table(omnibus_test, file = "omnibus_tukey_ofav_genotrt_koralmd3d.txt", sep = "\t")
...
write.table(tukey_stats, file = "ofav_SA_tukey_koralmd3d.txt", sep = "\t")
...
kw <- pairwise.wilcox.test(sa$delta, sa$geno_trt,
                           p.adjust.method = "fdr")
write.table(kw$p.value, file = "ofav_SA_kruskal_koralmd3d.txt", sep = "\t")
...
ggsave(filename="sa_boxplot_ofav_koralmd3d.png", plot=boxplot, device="png",  width = 6, height = 5, dpi=500)
...
write.table(tukey_stats, file = "apal_geno_SA_koralmd3d.txt", sep = "\t")
