...
aov.model <- aov(delta2 ~ geno_trt, data = sa)
...
write.table(omnibus_test, file = "omnibus_tukey_apal_genotrt_lugol.txt", sep = "\t")
...
write.table(tukey_stats, file = "apal_SA_tukey_lugol.txt", sep = "\t")

#should we use kruskal test instead?
kruskal.test(delta2 ~ geno_trt, data = sa) #write this down somewhere

kw <- pairwise.wilcox.test(sa$delta2, sa$geno_trt,
                           p.adjust.method = "fdr")
write.table(kw$p.value, file = "apal_SA_kruskal_lugol.txt", sep = "\t")



colors = c( '#0045EE', '#FBD92E', '#E26060', '#BE0032', '#74A4D1', '#106EC8', '#70BD64','#1A9906')
...
boxplot <- ggplot(sa, aes(x=Trt, y=delta, fill=Trt)) +
  ...
  labs(x="Treatment Type", y = "Change in Surface Area (mm²): T0 to T3") +
  stat_summary(geom = 'text', label = c("a", "b", "c", "a"), fun = max, vjust = -1, size = 3.5) 
boxplot

#ggsave(filename="sa_boxplot_apal_lugol.png", plot=boxplot, device="png",  width = 6, height = 5, dpi=500)
...
#take a look at the data
p <- ggplot(data=sa, aes(x=geno, y=delta2)) +
  geom_bar(stat = "summary", fun = "mean")
p
...
aov.model <- aov(delta2 ~ geno, data = sa)
summary(aov.model)
tukey_stats <- TukeyHSD(aov.model)
...
write.table(tukey_stats, file = "apal_geno_SA_lugol.txt", sep = "\t")
